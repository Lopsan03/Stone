...
export function Header({ userAddress = '0x71C...4f2e' }: { userAddress?: string }) {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-bento-card h-16 border-b border-bento-border px-6 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        <span className="text-[11px] font-bold bg-[#1e1b4b] text-[#c7d2fe] border border-[#312e81] px-2.5 py-1 rounded-full">
          Monad Testnet
        </span>
        ...
        <div className="flex items-center gap-3">
          {userAddress ? (
            ...
            <Button className="h-9 rounded-lg bg-bento-primary px-6 text-xs font-bold text-white shadow-lg shadow-indigo-500/20">
              Connect Wallet
            </Button>
          )}
        </div>
